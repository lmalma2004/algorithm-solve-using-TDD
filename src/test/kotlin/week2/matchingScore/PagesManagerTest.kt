...
internal class PagesManagerTest {
    ...
    fun `모든 웹페이지들의 점수를 계산해야 한다`() {
        ...
        val inputPages = TreeMap<Page, Boolean>()
        inputPages[Page.create(inputPage0, 0)] = true
        inputPages[Page.create(inputPage1, 1)] = true
        inputPages[Page.create(inputPage2, 2)] = true
    ...
    fun `매칭점수가 가장 높은 웹페이지가 여러개라면, 인덱스가 가장 낮은 웹페이지를 반환해야 한다`() {
        ...
        inputPages[Page("", 0, 3.0, 1.5)] = true
        assertEquals(0, pagesManager.getPageIdxOfMaxScore(inputPages))

        inputPages[Page("", 1, 3.0, 1.5)] = true
        assertEquals(0, pagesManager.getPageIdxOfMaxScore(inputPages))

        inputPages[Page("", 2, 4.0, 1.5)] = true
        assertEquals(2, pagesManager.getPageIdxOfMaxScore(inputPages))

        inputPages[Page("", 3, 4.0, 1.5)] = true
        assertEquals(2, pagesManager.getPageIdxOfMaxScore(inputPages))
    }
}
